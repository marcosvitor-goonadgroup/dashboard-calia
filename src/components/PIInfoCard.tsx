...
const PIInfoCard = ({ numeroPi, campaignData = [] }: PIInfoCardProps) => {
  ...
  const veiculacaoPorVeiculoTipo = useMemo(() => {
    ...
    piGrouped.forEach((piData) => {
      ...
      // Tenta match exato por veículo+tipo
      let match = realizadoGrouped.get(`${veiculoKey}|${tipoKey}`);
      let matchedByVehicle = !!match;

      // Se não encontrou, tenta apenas pelo tipo de compra
      if (!match) {
      ...
      // Último fallback: usa totais gerais quando há apenas uma linha de PI
            ...
            {totaisRealizados.realizado > 0 && (
              <div className="mt-2 pt-2 border-t border-gray-200">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded ${pacingOk ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                ...
                <p className={`text-sm font-semibold ${pacingOk ? 'text-green-700' : 'text-red-700'}`}>
                  ...
                  <span className="text-xs ml-1">({((totaisRealizados.realizado / totalInvestimento) * 100).toFixed(1)}% do total)</span>
                </p>
              </div>
            )}
          </div>
                  ...
                  <div key={index} className="border-l-4 border-blue-500 pl-3 py-2 bg-gray-50 rounded">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-1">
                        <span className={`px-2 py-0.5 text-xs font-semibold rounded ${investOk ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                          {investOk ? '✓ Invest.' : '! Invest.'}
                        </span>
                        {(isCPM || isCPC) && item.quantidade > 0 && (
                          <span className={`px-2 py-0.5 text-xs font-semibold rounded ${volumeOk ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className={`text-sm font-semibold ${investOk ? 'text-green-700' : 'text-red-700'}`}>
                          {fmt(item.realizado)}
                          {item.previsto > 0 && (
                            <span className="text-xs ml-1">({((item.realizado / item.previsto) * 100).toFixed(1)}%)</span>
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    {(isCPM || isCPC) && item.quantidade > 0 && (
                      <div>
                        ...
                        <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            className={`absolute h-full rounded-full transition-all ${volumeOk ? 'bg-green-500' : 'bg-blue-500'}`}
